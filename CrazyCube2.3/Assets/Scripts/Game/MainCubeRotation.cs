...
public class MainCubeRotation : MonoBehaviour 
{
	...
	void Update () 
    {
        ...
        {
            transform.Rotate((Input.GetTouch(0).deltaPosition.y * sensitivityY / 10 * Time.deltaTime), (Input.GetTouch(0).deltaPosition.x * -sensitivityX / 10 * Time.deltaTime), ((Input.GetTouch(1).deltaPosition.x * -sensitivityX / 10 * Time.deltaTime) + (Input.GetTouch(1).deltaPosition.y * -sensitivityY / 10 * Time.deltaTime)) / 2, Space.World);
        }
	}
}
